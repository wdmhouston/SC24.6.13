...
#
    ...
    _traindata_path = "/app/project/data/data"
    #training data folder
    
    _validationdata_path = "/app/project/data/data2"
    #validation data folder

    _testdata_path = "/app/project/data/data3"
    #test data folder. 
    ...
    def __init__(self):
        ...
        #get class labels
        dataset = tf.keras.preprocessing.image_dataset_from_directory(self._traindata_path,
                                                               seed=2509,
                                                               image_size=(224, 224),
                                                               batch_size=32)
    ...
    #train model from data folders
        dataset = tf.keras.preprocessing.image_dataset_from_directory(self._traindata_path,
                                                               ...
                                                               image_size=(224, 224),
                                                               batch_size=32)
        dataset2 = tf.keras.preprocessing.image_dataset_from_directory(self._validationdata_path,
                                                              seed=2509,
                                                              image_size=(224, 224),
                                                              shuffle=False,
                                                              batch_size=32)
        self._model = Sequential()
        self._model.add(Conv2D(32, kernel_size = (3, 3), activation='relu', input_shape=(224,224,3)))
        ...
        history = self._model.fit(x=dataset, epochs= 20, validation_data=dataset2)
        self._model.save(self._model_path + '/model.h5')
        return "model trained. model is saved to " + self._model_path + "/model.h5"
    ...
    #predict from existing model     
        ...
        return {'image': image_path, 'result': self._class_labels[np.argmax(pred)]}
        #return "actural object: " + image_path.split("/")[-2] + '   predicted object:' + self._class_labels[np.argmax(pred)]
